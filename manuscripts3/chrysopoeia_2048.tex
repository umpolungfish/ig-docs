...
\begin{abstract}
Companion to \cite{StarkHilbert12}: an explicit program for the $d=2048$
diagonal moduli $N_k=|\psi_k|^2$, reporting what direct computation has
settled and what remains open.

Five results. (1) The $a=0$ stratum is flat autocorrelation on the real moduli,
verified on $d=12$ to residual $\sim 10^{-58}$. (2) Unconstrained recovery is
blocked at a genuine local minimum (residual $3.9\times 10^{-3}$). (3) The
moduli field is the real index-$2$ subfield, of degree $2^{27}$, of the ray
class field of conductor $(d)\,\infty_1\infty_2$, calibrated on $d=12$ (moduli
of degree $4$ or $8$ generating a degree-$16$ field) with unramified ascent
verified to degree $128$. (4) Stark-unit $L$-values supply the moduli without building the full
field; validated end to end on $d=12$. (5) An integer norm sieve separates
degenerate $S$-unit aliases by discriminant $-\md$. Still open: the reduced
character set for the $1024$ moduli and unconditional existence.
\end{abstract}
...
In the Weyl--Heisenberg (WH) construction one seeks a single fiducial vector
...
open in general and verified numerically through $d\approx 2400$.

Appleby, Bengtsson, Flammia, and collaborators
\cite{Appleby2013,Appleby2017,AFMY2020} established that the fiducial
coordinates are not arbitrary transcendentals but algebraic numbers of
remarkable arithmetic: they lie in specific ray class fields of the real
quadratic field $\Fd = \mathbb{Q}(\sqrt{(d+1)(d-3)})$, tying SIC existence to
the real-quadratic case of Hilbert's twelfth problem. The companion paper
\cite{StarkHilbert12} formalizes this chain in \Lean{} and, for the frontier
dimension $d=2048=2^{11}$, proves the transport apparatus sorry-free while
leaving unconditional existence as the one open node.

This paper is the constructive counterpart. Rather than transport a fiducial
that is assumed to exist, we ask directly how the moduli
$N_k = |\psi_k|^2$ are to be constructed as exact algebraic numbers, and we
report a program that is complete in its components. We restrict throughout to
the diagonal $a=0$ stratum, which we show is governed entirely by the real
moduli, and we defer the phase strata to sequel work.

The account is deliberately empirical. Each field is computed in PARI/GP
\cite{PARI}, each identity is exhibited numerically to stated precision, and the
one bare-metal computation, the norm sieve, is executed and its output
...
$\operatorname{disc}\Fd = \md$ (as $\md\equiv 1 \bmod 4$). Computation in PARI/GP
gives class number $h(\Fd)=64$, class group $\mathbb{Z}/32\times\mathbb{Z}/2$,
and regulator $R \approx 7.6241$.
...
\end{gathered}
\]
Three arithmetic coincidences organize everything that follows.

\begin{enumerate}
\item The SIC denominator is a generator norm: $\Norm(g_4)=d+1$, so
$1/(d+1)$, the value that defines equiangularity, is the reciprocal of the field
norm of $g_4$.
\item The fundamental unit is the mean modulus: at the principal real embedding
...
of $\varepsilon$.
\item The discriminant factors through the dimension:
$\md = (d+1)(d-3)$, with $d+1=3\cdot 683$ and $d-3 = 5\cdot 409$, so the two
signed prime discriminants that carry the class-group structure are exactly the
factors of $d\pm$ small.
\end{enumerate}
...
\begin{proof}
By the Wiener--Khinchin identity $|\nu_{0,b}|^2 = \sum_m C_m\,\omega^{bm}$, so the
...
conjugates. And the whole reformulation is exact.
...
Before constructing the moduli algebraically, we record why they cannot be
obtained by numerical search, which is the natural first attempt and the one
that has stalled the frontier. We formed the full SIC objective
...
$8$, but together they generate the full degree-$16$ field. Second, real is not
totally real: the totally-real ray class field of conductor $(12)$ (degree $8$
over $\mathbb{Q}$, ray class group $(\mathbb{Z}/2)^2$) is a proper subfield of
the moduli field, and the archimedean ramification carries the rest. The
...
moduli. Applying it to $d=2048$, PARI/GP gives ray class group
\[
...
polynomial, with order $2^{27}$ coefficients, is not a writable object. This is
the central constraint the remainder of the paper is built around.
...
Since the degree-$2^{26}$ moduli field cannot be written as a polynomial, the
...
archimedean ramification, carry the remaining index: exactly the regime of the
mixed-signature Stark conjecture this program assumes.
\end{proposition}
...
\paragraph{Established by computation.}
\begin{itemize}
\item The $a=0$ stratum is the flat autocorrelation condition of
\Cref{prop:autocorr}, verified against $d=12$ to residual near $10^{-58}$
(\Cref{rem:d12-autocorr}).
\item The numerical route is structurally blocked by a genuine local minimum
(\Cref{prop:spurious}).
\item The moduli field is the real index-$2$ subfield of the
...
Hilbert class field of degree $128$ (\Cref{sec:tower}).
\item The value route via Stark units and $L'(0,\chi)$ produces the moduli field
without a prohibitive polynomial, validated end to end on $d=12$
(\Cref{prop:portals-d12}).
\item The recognition degeneracy is resolved by the exact norm sieve
(\Cref{prop:sieve}), implemented and executed.
\end{itemize}

\paragraph{Open.}
\begin{itemize}
\item The reduced set of ray class characters whose $s=0$ derivatives determine
the $1024$ independent $a=0$ moduli. This is the one computation between the
present apparatus and the explicit moduli.
\item The ramified ascent of the tower above the Hilbert class field, of index
$2^{20}$, if the reduced character route requires the full conductor.
\item Consequent to the above, the unconditional existence
$\texttt{SICPOVM\_Exists}\,2048$, which remains the open Zauner conjecture of the
companion paper \cite{StarkHilbert12}, and the phase strata $a\neq 0$ beyond the
diagonal.
\end{itemize}

The program is closed in its parts and validated on the dimension where the
answer is known. The gold is named, covered in 2048 leaves, and the furnace is built; the reduced portal
computation is the last distillation, and it is a computation, not a wall.
...
C.\ L.\ Mills, \emph{SIC-POVMs, a Stark Conjecture, and the Twelfth Problem:
